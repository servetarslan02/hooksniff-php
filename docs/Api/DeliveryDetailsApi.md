# OpenAPI\Client\DeliveryDetailsApi
...
```php
webhooksIdAttemptsAttemptIdGet($id, $attempt_id)
```
...
$apiInstance = new OpenAPI\Client\Api\DeliveryDetailsApi(
...
try {
    $apiInstance->webhooksIdAttemptsAttemptIdGet($id, $attempt_id);
} catch (Exception $e) {
    echo 'Exception when calling DeliveryDetailsApi->webhooksIdAttemptsAttemptIdGet: ', $e->getMessage(), PHP_EOL;
}
```
...
try {
    ...
    echo 'Exception when calling DeliveryDetailsApi->webhooksIdDetailsGet: ', $e->getMessage(), PHP_EOL;
}
```
